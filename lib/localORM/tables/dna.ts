...
export class DNA extends BaseEntry {
    ...
    public copyToClipboard(): void {
        ...
        builder.addLine("<!-- Titel: [FSD-FA] Akz. " + this.akz + " DNA Probe - " + this.dnas.map((x) => x.name).join(", ") + " -->");
        builder.addLine("<!-- Titel: DNA Probe - " + this.dnas.map((x) => x.name).join(", ") + " -->");
        builder.addLine("");
        builder.addLine("`Datum:` " + getFormattedDate(this.createdAt, "DD.MM.YYYY - HH:mm Uhr") + "<br>");
        ...
        for (const dna of this.dnas) {
            builder.addLine(`- ${dna.dnaType + " ".repeat(20 - dna.dnaType.length)}${dna.name.toString()}`);
        }
