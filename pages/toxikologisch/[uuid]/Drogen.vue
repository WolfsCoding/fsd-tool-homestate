...
import { type DrugData, drugsData } from '@/data/Drugs';
...
async function handleRemoveDrug(drugId: string) {
  ...
  removeDrug(analyse.value.id, drugId);
...
function selectDrug() {
  const drug = drugs.value.find((x) => x.name === createDialog.value.name);
  createDialog.value.variables =
    drug?.variables.map((x) => ({ key: x.placeholder, name: x.name, value: '' })) || [];
}
</script>
                  ...
                  <CardContent>
                    <div class="grid gap-4 py-4">
                      ...
                      <p class="text-sm text-muted-foreground -mt-3">
                        Füge hier die generellen Werte der Analyse ein.
                      </p>
                      <div class="grid grid-cols-4 items-center gap-4">
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem
                              v-for="drug in drugs.map((x) => x.name)"
                              :key="drug"
                              :value="drug"
                            >
                              {{ drug }}
                      ...
                      <div class="grid grid-cols-4 items-center gap-4">
                        ...
                        <Input
                          ...
                          class="col-span-3"
                          type="text"
                          v-model="createDialog.unit"
                        />
                      </div>
                      <p
                        class="text-lg font-semibold leading-none tracking-tight"
                        v-if="createDialog.variables.length > 0"
                      >
                        Drogenspezifiche Werte
                      </p>
                      <p
                        class="text-sm text-muted-foreground -mt-3"
                        v-if="createDialog.variables.length > 0"
                      >
                        Füge hier die Drogenspezifiche Werte der Analyse ein.
                      </p>
                      <div
                        class="grid grid-cols-4 items-center gap-4"
                        v-for="variable in createDialog.variables"
                        :key="variable.key"
                      >
                  ...
                  <TableCell class="[--table-padding:0] pl-4">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent>
                        ...
                        <DropdownMenuItem @click="handleRemoveDrug(drug.id)"
