        ...
        <Table>
          ...
          <TableBody>
            <TableRow
              ...
              :key="analyse.akz"
              :href="'/dna/' + analyse.id + '/DNA'"
            >
              <TableCell> {{ analyse.akz }} </TableCell>
              <TableCell>
                {{ getFormattedDate(analyse.createdAt, 'DD.MM.YYYY hh:mm Uhr') }}</TableCell
              >
              ...
              <TableCell class="[--table-padding:0] pl-4">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    ...
                    <DropdownMenuItem @click="router.push('/dna/' + analyse.id + '/DNA')">
                      Öffnen
                    </DropdownMenuItem>
                    <DropdownMenuItem @click="analyse.copyToClipboard()"
                      >Analyse kopieren</DropdownMenuItem
                    >
                    <DropdownMenuItem @click="handleDeleteAnalyse(analyse.id)"
