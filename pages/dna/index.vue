        ...
        <div class="flex flex-col sm:gap-4 sm:py-4">
            <header class="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6">
                <Breadcrumb class="hidden md:flex">
                    <BreadcrumbList>
                        <BreadcrumbItem>
                            <BreadcrumbPage>DNA Proben</BreadcrumbPage>
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
            </header>
            <main class="grid flex-1 items-start gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="analyse in analysen.filter((x) => x.akz.toLowerCase().includes(props.search.toLowerCase()) || x.forName.toLowerCase().includes(props.search.toLowerCase()) || x.gutachter.toLowerCase().includes(props.search.toLowerCase()))" :key="analyse.akz" :href="'/dna/' + analyse.id">
                            ...
                            <TableCell class="[--table-padding:0] pl-4">
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent>
                                        ...
                                        <DropdownMenuItem @click="router.push('/dna/' + analyse.id)"> Öffnen </DropdownMenuItem>
                                        <DropdownMenuItem @click="analyse.copyToClipboard()">Analyse kopieren</DropdownMenuItem>
                                        <DropdownMenuItem @click="handleDeleteAnalyse(analyse.id)">Löschen</DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </TableCell>
                        </TableRow>
