...
async function handleAddEntry() {
  ...
  await addSparbuchEntry(
    new SparbuchEntry({
      datum: getFormattedDate(new Date(), 'DD.MM.YYYY hh:mm Uhr'),
      betrag:
        Math.round(
          (type.value === ENTRY_TYPES.Zinsen
            ? value - getSparbuchAmount().value
            : type.value === ENTRY_TYPES.Einzahlung
            ? value
        ...
        <div class="grid gap-4 py-4 grid-cols-12">
          ...
          <div class="col-span-3">
            <Input
              ...
              type="text"
              v-model="betrag"
              :placeholder="type == ENTRY_TYPES.Zinsen ? 'Betrag auf dem Sparbuch' : 'Betrag'"
            />
          </div>
          ...
          <TableBody>
            <TableRow v-for="entry in sparbuchEntrys.filter((x) => true).slice(0, 20)">
              ...
              <TableCell
                :class="{
                  'text-green-500':
                    entry.type === ENTRY_TYPES.Zinsen || entry.type === ENTRY_TYPES.Einzahlung,
                  'text-red-500': entry.type === ENTRY_TYPES.Auszahlung,
                }"
              >
                {{ entry.type === ENTRY_TYPES.Auszahlung ? '-' : '+' }}
                {{ entry.type === ENTRY_TYPES.Auszahlung ? entry.betrag * -1 : entry.betrag }} $
              </TableCell>
